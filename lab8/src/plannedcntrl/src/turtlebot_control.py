...
import sys
import numpy as np
from geometry_msgs.msg import TransformStamped, PoseStamped, Twist, Point, PointStamped
...
def controller(waypoint):
  ...
  # waypoint - point to reach/pass through
  ...
  while not rospy.is_shutdown():
    try:
      ...
      # proportional term
      proportional = np.dot(Kp, error).squeeze()   # shape: [x, y]
...
def planning_callback(msg: Point):
  print('planning callback executed')
  try:
